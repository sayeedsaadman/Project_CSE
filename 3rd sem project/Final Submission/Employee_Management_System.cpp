...
vector<pair<string, string>> admin(1000);
...
void save_registration_to_file() {
    ...
    for (int i = 0; i < admin.size(); i++) {
        file << admin[i].first << " " << admin[i].second << endl;
    }
    file.close();
}
...
void load_registration_from_file() {
    ...
    while (file >> username >> password) {
        admin.push_back({username, password});
    }
    file.close();
}
...
void Admin_login()
{
    ...
    getline(cin,password);
    for (int i = 0; i < admin.size(); i++) {
        if (admin[i].first == name && admin[i].second == password) {
            gotoxy(49, 19);
